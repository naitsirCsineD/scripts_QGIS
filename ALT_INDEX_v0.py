...
class AlterationIndicesFromLandsatIllum(QgsProcessingAlgorithm):
    ...
    def displayName(self):
        return self.tr("Índices de alteración G1(fe_index, clay_index, oh_index) G2(propylitic_index, silica_index, grossan_index) (Landsat + DTM + corrección de iluminación)")
    ...
    def shortHelpString(self):
        return self.tr(
            "Genera varios índices de alteración (Fe, arcillas, OH y algunos ratios espectrales) "
            "a partir de Landsat 8, corrigiendo por iluminación derivada de un DTM. "
            "El DTM puede estar en otro CRS: se reproyecta al CRS de las bandas. "
            "Los 6 índices se empaquetan en 2 ráster de 3 bandas."
        )
    ...
    def processAlgorithm(self, parameters, context: QgsProcessingContext, feedback):
        ...
        # 8. NDVI (por si quieres seguir enmascarando)
        ...
        # 9. DEFINIMOS AQUÍ LOS OTROS ÍNDICES DE ALTERACIÓN
        # puedes ajustar las fórmulas a tu criterio
        index_defs = [
            # nombre, expr, (A,B)
            ("fe_index", f"(B > 0.0001) * (A / (B + {eps}))", b4_corr, b2_corr),     # Fe férrico aprox
            ("clay_index", f"(B > 0.0001) * (A / (B + {eps}))", b6_corr, b7_corr),   # Arcillas SWIR1/SWIR2
            ("oh_index", f"(B > 0.0001) * (A / (B + {eps}))", b6_corr, b5_corr),     # OH SWIR1/NIR
            ("propylitic_index", f"(B > 0.0001) * (A / (B + {eps}))", b5_corr, b4_corr),  # NIR/RED
            ("silica_index", f"(B > 0.0001) * (A / (B + {eps}))", b6_corr, b2_corr),      # SWIR1/BLUE
            ("gossan_index", f"(B > 0.0001) * (A / (B + {eps}))", b4_corr, b6_corr),      # RED/SWIR1
        ]
        ...
        # 10. empaquetar en ráster de 3 bandas
        # QGIS/gdal:merge no pone nombres de banda, pero las bandas quedan en orden.
